...
## 現状
- モバイル media query は `@media (max-width: __MOBILE_BREAKPOINT_PX__)` で記述し、Vite で `MOBILE_BREAKPOINT_PX` に展開する。
- `BREAKPOINT_SYNC_NOTE` 同期注記は以下に配置済み。
...
- 変換プラグインは `tools/vite-breakpoint-token-plugin.ts`。
- 既存の同期チェックは `npm run lint:breakpoints` で実行可能。

## 運用ルール
1. 既存のモバイル判定を変更する場合は、必ず `src/constants/layout.ts` の `MOBILE_BREAKPOINT_PX` を起点にする。
2. CSS 側では `@media (max-width: __MOBILE_BREAKPOINT_PX__)` を使用し、`768px` 直書きを禁止する。
3. `@media (max-width: __MOBILE_BREAKPOINT_PX__)` の直前には `BREAKPOINT_SYNC_NOTE` コメントを維持する。
4. 新規にモバイル系メディアクエリを追加する場合は、既存コメントと同一文面で注記を追加する。
5. 実装後は `npm run lint:breakpoints` を実行し、注記・トークン・禁止値の不整合がないことを確認する。
...
4. 対応する CSS は `@media (max-width: __MOBILE_BREAKPOINT_PX__)` を使用する。
5. `npm run lint:breakpoints` を実行し、同期漏れがないことを確認する。

## チェック観点
- 変更した px 値が TS 定数とビルド後 CSS で一致している。
- メディアクエリ追加時に同期注記が欠落していない。
- `@media (max-width: 768px)` の直書きが再導入されていない。
